...
class GitLabLoginHandler(BaseHandler, tornado.auth.OAuth2Mixin):
    ...
    def _on_auth(self, user):
        ...
        if allowed_groups:
            ...
            response = yield self.get_auth_http_client().fetch(
                "https://gitlab.com/api/v4/groups?min_access_level=%s"
                % (min_access_level,),
                headers={
                    "Authorization": "Bearer " + access_token,
                    "User-agent": "Tornado auth",
...
class OktaLoginHandler(BaseHandler, tornado.auth.OAuth2Mixin):
    ...
    def _OAUTH_AUTHORIZE_URL(self):
        return "{}/as/authorization.oauth2".format(self.base_url)

    @property
    def _OAUTH_ACCESS_TOKEN_URL(self):
        return "{}/as/token.oauth2".format(self.base_url)

    @property
    def _OAUTH_USER_INFO_URL(self):
        return "{}/idp/userinfo.openid".format(self.base_url)

    @property
    def _OAUTH_USER_GROUP(self):
        return os.environ.get("FLOWER_OAUTH2_GROUP_LIST").split(";")
    ...
    def get(self):
        redirect_uri = self.settings[self._OAUTH_SETTINGS_KEY]["redirect_uri"]
        if self.get_argument("code", False):
            expected_state = (self.get_secure_cookie("oauth_state") or b"").decode(
                "utf-8"
            )
    ...
    def _on_auth(self, access_token_response):
        ...
        email_verified = decoded_body.get("email_verified") and re.match(
            self.application.options.auth, email
        )
        access_groups = decoded_body.get("memberof")
        if not any(group in self._OAUTH_USER_GROUP for group in access_groups):
            message = (
                "Access denied. Please use another account or "
                "ask your admin to add your email to flower --auth."
            )
        ...
        next_ = (
            self.get_argument("next", self.application.options.url_prefix or "/") + "/"
        )
        if self.application.options.url_prefix and next_[0] != "/":
            next_ = "/" + next_
        self.redirect(next_)
